...
Each class session has a folder under `lectures/`, named by date — with a suffix such
...
| `history.R` | the R code run live in class ← input, optional |
| `R-console.txt` | that code *and* its output, from the R console ← input, optional |
...
microphone hears "read CSV the path", and the details disappear. When a session folder has a
...
python3 $cn generate --only lectures/2026-09-15 --prompt glossary
python3 $cn generate --only lectures/2026-09-15 --force --model sonnet
python3 $cn payload --only lectures/2026-09-15 > message.txt
```
...
These transcripts include your voices. Students appear in the transcript
